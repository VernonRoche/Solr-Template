<?php
require_once $_SERVER['DOCUMENT_ROOT'] . "/source/PHP/solr_bootstrap.php";
...
if ($autosuggest == True) {
    ...
    try {
        ...
        $response_items = send_get_response($start, $solr_query, $boost_query, $client);
        $autosuggest_list=[];
        foreach ($response_items['docs'] as $item){
            $autosuggest_item['title']=$item['title'];
            $autosuggest_item['absolute_path']=$item['absolute_path'];
 ...
 */
function send_get_response($start, $query, $boost_query, $client)
{
